...
export const fetchSheetDataCSV = async () => {
  try {
    ...
    // Os dados anuais estão nas linhas 2 a 7 (índices 1 a 6) nas colunas D e E
    ...
    // Processa dados anuais (colunas D e E, linhas 2 a 7)
    const annualData = []
    // Os dados anuais estão nas linhas 1 a 6 (índices 1 a 6)
    // Coluna E (índice 4) contém o ano, coluna F (índice 5) contém a rentabilidade
    for (let i = 1; i <= 6; i++) { // Loop de 2025 a 2030 (linhas 2 a 7 da planilha)
      const line = lines[i].trim()
      if (line) {
        const values = line.split(',')
        // O ano está na coluna E (índice 4)
        const ano = values[4] ? values[4].replace(/"/g, '').trim() : ''
        // A rentabilidade anual está na coluna F (índice 5)
        const rentabilidadeAnual = parseFloat(values[5] ? values[5].replace(/"/g, "").replace("%", "").replace(",", ".") : '0') || 0
